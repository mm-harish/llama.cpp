...
static ggml_backend_redefine_context *ggml_redefine_init(ggml_backend_dev_t dev);
...
static ggml_backend_i ggml_backend_redefine_i = {
    ...
    /* .set_tensor_async        = */
    NULL, /* ggml_backend_redefine_set_tensor_async
           */
          /* .get_tensor_async        = */
    NULL, /* ggml_backend_redefine_get_tensor_async
           */
    /* .set_tensor_2d_async     = */ NULL,
    /* .get_tensor_2d_async     = */ NULL,
    /* .cpy_tensor_async        = */
    NULL, /* ggml_backend_redefine_cpy_tensor_async
           */
...
ggml_backend_t ggml_backend_redefine_init(void) {
  ggml_backend_dev_t dev =
      ggml_backend_reg_dev_get(ggml_backend_redefine_reg(), 0);
  ggml_backend_redefine_context *backend_ctx = ggml_redefine_init(dev);

  ggml_backend_t backend =
      new ggml_backend{/* .guid    = */ NULL,
                       /* .iface   = */ ggml_backend_redefine_i,
                       /* .device  = */ dev,
                       /* .context = */ backend_ctx};
...
const char *ggml_backend_redefine_device_name(ggml_backend_dev_t dev) {
  const auto *dev_ctx =
      static_cast<const ggml_backend_redefine_device_context *>(dev->context);
  return dev_ctx->device_name.c_str();
}

const char *ggml_backend_redefine_device_description(ggml_backend_dev_t dev) {
  const auto *dev_ctx =
      static_cast<const ggml_backend_redefine_device_context *>(dev->context);
...
ggml_backend_redefine_device_type(ggml_backend_dev_t dev) {
  GGML_UNUSED(dev);
  return GGML_BACKEND_DEVICE_TYPE_GPU;
}

void ggml_backend_redefine_device_get_memory(ggml_backend_dev_t dev, size_t *free,
                                             size_t *total) {
  const auto *dev_ctx =
      static_cast<const ggml_backend_redefine_device_context *>(dev->context);
  if (total) {
...
struct ggml_backend_device_i ggml_backend_redefine_device_i = {
    /* .get_name             = */ ggml_backend_redefine_device_name,
    /* .get_description      = */ ggml_backend_redefine_device_description,
    /* .get_memory           = */ ggml_backend_redefine_device_get_memory,
    /* .get_type             = */ ggml_backend_redefine_device_type,
    /* .get_props            = */ NULL,
    /* .init_backend         = */ NULL,
...
// Look for available and suitable devices (isasim, fsim, or redefine from pocl_platform).
...
ggml_redefine_probe_devices(ggml_backend_reg *reg) {
  ...
  cl_context_properties properties[] = {
      (intptr_t)CL_CONTEXT_PLATFORM, (intptr_t)selected_platform, 0};
  CL_CHECK((
      shared_context =
  ...
  for (cl_device_id device : matching_devices) {
    ...
    GGML_LOG_INFO("ggml_redefine: device: '%s (%s)'\n",
            device_name.c_str(), device_version.c_str());
